    ...
    <div class="container">
        <div class="card py-2 px-4 mt-2 w-10/12 mx-auto">
            ...
            <div class="card-header">
                <h2>{{ __('Update Penta Force Ai Slider') }}</h2>
            </div>

            <div class="card-body">
                <form action="{{ route('admin.settings.frontend.pentaforceupdatepost') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $pentaForceAiSlides->id }}">
                    ...
                    <div class="mb-3 input-box">
                        ...
                        <input class="form-control" type="text" id="title" name="title" required value="{{ $pentaForceAiSlides->title }}">
                    </div>
                    <div class="mb-3 input-box">
                        <label for="color" class="form-label">{{ __('Color') }}</label>
                        <input style="height: 50px" type="color" name="color" class="form-control" value="{{ $pentaForceAiSlides->color }}">
                    </div>
                    <div class="mb-3 input-box">
                        <label for="description" class="form-label">{{ __('Description') }}</label>
                        <textarea class="form-control" type="text" id="description" name="description" rows="3" required>{{$pentaForceAiSlides->description}}</textarea>
                    </div>
