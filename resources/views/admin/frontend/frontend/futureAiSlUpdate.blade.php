    ...
    <div class="container">
        <div class="card p-4 mt-2 w-1/2 mx-auto">
            <div class="card-header">
                <h2>{{ __('Update Added Future of AI Slide Item') }}</h2>
            </div>

            <div class="card-body">
                <form action="{{ route('admin.settings.frontend.futureaislupdatepost') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{$webFutureAiContent->id}}">
                    ...
                    <div class="mb-3 input-box">
                        ...
                        <input class="form-control" type="text" id="title" name="title" required value="{{$webFutureAiContent->title}}">
                    </div>
                    <div class="mb-3 input-box">
                        <label for="slug" class="form-label">{{ __('Slug') }}</label>
                        <input class="form-control" type="text" id="slug" name="slug" required value="{{$webFutureAiContent->slug}}">
                    </div>
                    <div class="mb-3 input-box">
                        <label for="color" class="form-label">{{ __('Color') }}</label>
                        <input type="color" class="w-full" id="color" name="color" required value="{{$webFutureAiContent->color}}">
                    </div>
                    <div class="mb-3 input-box">
                        <label for="description" class="form-label">{{ __('Description') }}</label>
                        <textarea class="form-control" type="text" id="description" name="description" rows="3" required>{{$webFutureAiContent->description}}</textarea>
                    </div>
