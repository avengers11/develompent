...
class PageUserController extends Controller
{
    public function pageContent($slug)
    {
        ...
        if ($page) {
            return view('page.index', compact('page', 'webFooterMenus', 'webFooterIconMenus'));
        } else {
            abort(404);
        }
    }
    ...
    public function pagePrivacy()
    {
        ...
        if ($settings) {
            return view('page.privacy_policy', compact('webFooterMenus', 'webFooterIconMenus', 'page'));
        } else {
            abort(404);
        }
    }
    ...
    public function pageTerms()
    {
        ...
        if ($settings) {
            return view('page.terms', compact('webFooterMenus', 'webFooterIconMenus', 'page'));
        } else {
            abort(404);
        }
    }
    ...
    public function pageCookies()
    {
        ...
        $page = (object)[];
        $page->status = $settings->privacy_enable;
        $page->title = __('COOKIE POLICY');
        $page->content = $settings->cookies;
        // Check page status
        if (!$page->status) {
            abort(404);
        }
        if ($settings) {
            return view('page.cookies', compact('webFooterMenus', 'webFooterIconMenus', 'page'));
        } else {
            abort(404);
        }
    }
